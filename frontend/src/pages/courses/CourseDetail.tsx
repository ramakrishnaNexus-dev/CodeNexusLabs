...
const CourseDetail = () => {
  ...
  useEffect(() => {
    ...
    if (isAuthenticated && allTopics.length > 0) {
      ...
      axios.get(`https://codenexuslabs.onrender.com/api/v1/progress/course/${id}`, {
  ...
  const markCompleted = async (i: number) => {
    ...
    try {
      const token = localStorage.getItem('token') || sessionStorage.getItem('token') || '';
      await axios.post(`https://codenexuslabs.onrender.com/api/v1/progress/${topic.id}`, { courseId: Number(id) }, { headers: { Authorization: `Bearer ${token}` } });
