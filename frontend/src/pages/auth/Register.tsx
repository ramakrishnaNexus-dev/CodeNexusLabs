  ...
  if (done) return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-50 via-white to-indigo-50 px-4">
      <div className="text-center max-w-md">
        ...
        <p className="text-gray-500 mb-2">Welcome aboard!</p>
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-50 via-white to-indigo-50 px-4 py-8">
      <div className="w-full max-w-[900px] grid md:grid-cols-2 bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden">
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-3.5">
            <div><label className="block text-xs font-semibold text-gray-700 mb-1">Full Name</label>
              <div className="relative"><User className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400" />
                <input type="text" placeholder="John Doe" className={`input-field pl-9 ${errors.fullName ? 'input-error' : ''}`}
                  {...register('fullName', { required: 'Name is required', minLength: { value: 2, message: 'At least 2 characters' } })} /></div>
              {errors.fullName && <p className="error-text">{errors.fullName.message}</p>}</div>
            ...
            <div><label className="block text-xs font-semibold text-gray-700 mb-1">Password</label>
              <div className="relative"><Lock className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400" />
                <input type={show ? 'text' : 'password'} placeholder="Create strong password" className={`input-field pl-9 pr-9 ${errors.password ? 'input-error' : ''}`}
                  {...register('password', { required: 'Password is required', minLength: { value: 8, message: 'At least 8 characters' }, validate: { hasUpper: v => /[A-Z]/.test(v) || 'Need uppercase', hasLower: v => /[a-z]/.test(v) || 'Need lowercase', hasNumber: v => /[0-9]/.test(v) || 'Need number' } })} />
              ...
              {password && (<div className="mt-1.5"><div className="flex gap-1 mb-1">{[...Array(4)].map((_, i) => (<div key={i} className={`h-1 flex-1 rounded-full ${i < passwordStrength() ? strengthColor[passwordStrength()] : 'bg-gray-200'}`} />))}</div><p className="text-[10px] text-gray-500">Strength: <span className="font-medium">{strengthLabel[passwordStrength()]}</span></p></div>)}
              {errors.password && <p className="error-text">{errors.password.message}</p>}</div>

            <div><label className="block text-xs font-semibold text-gray-700 mb-1">Confirm Password</label>
              <div className="relative"><Lock className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400" />
                <input type="password" placeholder="Repeat your password" className={`input-field pl-9 ${errors.confirmPassword ? 'input-error' : ''}`}
                  {...register('confirmPassword', { required: 'Confirm your password', validate: v => v === password || 'Passwords do not match' })} /></div>
              {errors.confirmPassword && <p className="error-text">{errors.confirmPassword.message}</p>}</div>
        ...
        <div className="hidden md:flex flex-col justify-center p-10 bg-gradient-to-br from-indigo-600 via-purple-600 to-pink-500 text-white">
          ...
          <p className="text-white/70 text-sm leading-relaxed mb-6">Start your journey today. Join thousands of learners advancing their tech careers.</p>
