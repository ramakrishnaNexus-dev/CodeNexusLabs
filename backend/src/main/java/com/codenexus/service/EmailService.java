...
public class EmailService {
    ...
    @Value("${admin.email:codenexuslabs.dev@gmail.com}")
    ...
    public void sendWelcomeEmail(String to, String name) {
        try {
            ...
            helper.setText(getWelcomeEmailHtml(name), true);
    ...
    public void sendAdminNotification(String newUserName, String newUserEmail) {
        try {
            ...
            helper.setText(getAdminNotificationHtml(newUserName, newUserEmail), true);
            mailSender.send(message);
    ...
    public void sendPasswordResetEmail(String to, String name, String resetToken) {
        try {
            ...
            helper.setFrom(fromEmail);
            helper.setTo(to);
            helper.setSubject("Reset Your Password - CodeNexusLabs");

            String resetUrl = "https://code-nexus-labs.vercel.app/reset-password?token=" + resetToken;
            helper.setText(getPasswordResetHtml(name, resetUrl), true);
            mailSender.send(message);
    ...
    // Keep your existing HTML template methods here
    private String getWelcomeEmailHtml(String name) {
        return "<!DOCTYPE html><html><head><meta charset='UTF-8'></head>"
            + "<body style='margin:0;padding:0;background:#f3f4f6;font-family:Arial,sans-serif;'>"
            + "<table width='100%' cellpadding='0' cellspacing='0'>"
            + "<td><td align='center' style='padding:40px 20px;'>"
            ...
            + "<tr><td style='background:#eef2ff;padding:15px;text-align:center;border-radius:10px 0 0 10px;'><p style='margin:0;font-size:20px;font-weight:700;color:#4f46e5;'>50+</p><p style='margin:5px 0 0;font-size:11px;color:#6b7280;'>Expert Courses</p></td>"
            ...
            + "<table width='100%' cellpadding='0' cellspacing='0'>"
            + "<tr><td align='center' style='padding:20px 0 25px;'>"
    ...
    private String getAdminNotificationHtml(String userName, String userEmail) {
        return "<!DOCTYPE html><html><head><meta charset='UTF-8'></head>"
            ...
            + "<tr><td style='color:#6b7280;font-size:13px;'>Name:</td><td style='color:#1f2937;font-weight:600;'>" + userName + "</td></tr>"
            + "<tr><td style='color:#6b7280;font-size:13px;'>Email:</td><td style='color:#4f46e5;'>" + userEmail + "</td></tr>"
    ...
    private String getPasswordResetHtml(String name, String resetUrl) {
        return "<!DOCTYPE html><html><head><meta charset='UTF-8'></head>"
            ...
            + "<tr><td align='center' style='padding:40px 20px;'>"
            + "<table width='550' cellpadding='0' cellspacing='0' style='background:#ffffff;border-radius:16px;overflow:hidden;'>"
            + "<tr><td style='background:linear-gradient(135deg,#4f46e5,#6366f1);padding:30px;text-align:center;'>"
            + "<h2 style='color:#ffffff;margin:0;font-size:22px;'>Reset Your Password</h2>"
            + "</td>"
            + "</tr>"
            + "<tr>"
            + "<td style='padding:30px;'>"
            + "<p style='font-size:15px;color:#374151;'>Hi <strong>" + name + "</strong>,</p>"
            + "<p style='font-size:14px;color:#6b7280;'>We received a request to reset your password. Click below to create a new password:</p>"
            + "<div style='text-align:center;margin:30px 0;'>"
            + "<a href='" + resetUrl + "' style='display:inline-block;background:#4f46e5;color:#ffffff;text-decoration:none;padding:14px 35px;border-radius:10px;font-size:16px;font-weight:600;'>Reset Password</a>"
            + "</div>"
            + "<p style='font-size:13px;color:#9ca3af;'>This link expires in 1 hour. If you didn't request this, ignore this email.</p>"
            + "</td>"
            + "</tr>"
            + "</tr>"
            + "</td>"
